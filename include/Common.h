...
#define NDISRD_VERSION             0x04083000
...
#define NDISRD_MINOR_VERSION       0x0408
 ...
 * @param m_szAdapterNameList This is an array of adapter names. The size of the array is defined by the ADAPTER_LIST_SIZE constant, and each name is a string of characters with a maximum length defined by the ADAPTER_NAME_SIZE constant.
 ...
 * @param m_czCurrentAddress This is an array of current (configured) Ethernet addresses. Each address is represented as an array of bytes with a length defined by the ETHER_ADDR_LENGTH constant.
 ...
 * In the TCP_AdapterList_WOW64 structure, the m_nAdapterHandle field is an array of ULARGE_INTEGER to accommodate the larger pointer size in a 64-bit environment.
 *
 * @param m_nAdapterCount This field stores the number of adapters.
 * @param m_szAdapterNameList This is an array of adapter names. The size of the array is defined by the ADAPTER_LIST_SIZE constant, and each name is a string of characters with a maximum length defined by the ADAPTER_NAME_SIZE constant.
 * @param m_nAdapterHandle This is an array of adapter handles. These handles are key for any adapter relative operation.
 ...
 * @param m_Flags This field stores NDIS_PACKET flags. These flags provide information about the packet, such as whether it's a loopback packet or a broadcast packet.
 ...
 * WOW64 is a subsystem of the Windows operating system capable of running 32-bit applications and is included on all 64-bit versions of Windows.
 * In the INTERMEDIATE_BUFFER_WOW64 structure, the m_hAdapter and m_qLink fields are arrays of two handles to accommodate the larger pointer size in a 64-bit environment.
 *
 * @param m_hAdapter/m_qLink This is a union of two fields. m_hAdapter is a handle to the adapter. m_qLink is a link to the next packet in the queue.
 * @param m_dwDeviceFlags This field stores device-specific flags. These flags can be used to indicate whether the packet is on send or receive.
 * @param m_Length This field stores the length of the packet.
 ...
 * WOW64 is a subsystem of the Windows operating system capable of running 32-bit applications and is included on all 64-bit versions of Windows.
 *
 * @param Buffer This field is a ULARGE_INTEGER that contains the address of an INTERMEDIATE_BUFFER structure that contains the packet data and related information.
 */
 ...
 * @param EthPacket This field is an NDISRD_ETH_Packet structure that contains the packet data and related information. When reading, this structure will be filled with the packet data retrieved from the driver. When sending, this structure should be filled with the packet data to be sent.
 */
 ...
 * WOW64 is a subsystem of the Windows operating system capable of running 32-bit applications and is included on all 64-bit versions of Windows.
 * In the ETH_REQUEST_WOW64 structure, the hAdapterHandle field is a ULARGE_INTEGER and the EthPacket field is an NDISRD_ETH_Packet_WOW64 structure to accommodate the larger pointer size in a 64-bit environment.
 *
 * @param hAdapterHandle This field is a ULARGE_INTEGER that represents a handle to the adapter. It's used to identify the adapter that the packet is being read from or sent to.
 * @param EthPacket This field is an NDISRD_ETH_Packet_WOW64 structure that contains the packet data and related information. When reading, this structure will be filled with the packet data retrieved from the driver. When sending, this structure should be filled with the packet data to be sent.
 */
 ...
 * @param EthPacket This is an array of NDISRD_ETH_Packet structures that contain the packet data and related information. When reading, these structures will be filled with the packet data retrieved from the driver. When sending, these structures should be filled with the packet data to be sent.
 */
 ...
 * WOW64 is a subsystem of the Windows operating system capable of running 32-bit applications and is included on all 64-bit versions of Windows.
 * In the ETH_M_REQUEST_WOW64 structure, the hAdapterHandle field is a ULARGE_INTEGER and the EthPacket field is an array of NDISRD_ETH_Packet_WOW64 structures to accommodate the larger pointer size in a 64-bit environment.
 *
 * @param hAdapterHandle This field is a ULARGE_INTEGER that represents a handle to the adapter. It's used to identify the adapter that the packets are being read from or sent to.
 * @param dwPacketsNumber This field stores the number of packets.
 * @param dwPacketsSuccess This field stores the number of successfully processed packets.
 * @param EthPacket This is an array of NDISRD_ETH_Packet_WOW64 structures that contain the packet data and related information. When reading, these structures will be filled with the packet data retrieved from the driver. When sending, these structures should be filled with the packet data to be sent.
 */
 ...
 * @param dwFlags This field stores the mode flags. These flags determine the working mode of the adapter. The flags can be a combination of the following values: MSTCP_FLAG_SENT_TUNNEL, MSTCP_FLAG_RECV_TUNNEL, MSTCP_FLAG_SENT_LISTEN, MSTCP_FLAG_RECV_LISTEN, MSTCP_FLAG_FILTER_DIRECT, MSTCP_FLAG_TUNNEL, MSTCP_FLAG_LISTEN, and MSTCP_FLAG_FILTER.
 */
 ...
 *
 * @param hAdapterHandle This field is a ULARGE_INTEGER that represents a handle to the adapter. It's used to identify the adapter that the mode is being set for.
 * @param dwFlags This field stores the mode flags. These flags determine the working mode of the adapter. The flags can be a combination of the following values: MSTCP_FLAG_SENT_TUNNEL, MSTCP_FLAG_RECV_TUNNEL, MSTCP_FLAG_SENT_LISTEN, MSTCP_FLAG_RECV_LISTEN, MSTCP_FLAG_FILTER_DIRECT, MSTCP_FLAG_TUNNEL, MSTCP_FLAG_LISTEN, and MSTCP_FLAG_FILTER.
 */
 ...
 * WOW64 is a subsystem of the Windows operating system capable of running 32-bit applications and is included on all 64-bit versions of Windows.
 * In the ADAPTER_EVENT_WOW64 structure, the hAdapterHandle and hEvent fields are ULARGE_INTEGER to accommodate the larger pointer size in a 64-bit environment.
 *
 * @param hAdapterHandle This field is a ULARGE_INTEGER that represents a handle to the adapter. It's used to identify the adapter that the event is being set for.
 ...
 * @param Oid This field stores the OID (Object Identifier) of the request. The OID is a code that identifies the network parameter that is being queried or set.
 ...
 * WOW64 is a subsystem of the Windows operating system capable of running 32-bit applications and is included on all 64-bit versions of Windows.
 * In the PACKET_OID_DATA_WOW64 structure, the hAdapterHandle field is a ULARGE_INTEGER to accommodate the larger pointer size in a 64-bit environment.
 *
 * @param hAdapterHandle This field is a ULARGE_INTEGER that represents a handle to the adapter. It's used to identify the adapter that the request is being made on.
 * @param Oid This field stores the OID (Object Identifier) of the request. The OID is a code that identifies the network parameter that is being queried or set.
 * @param Length This field stores the length of the data being queried or set.
 * @param Data This is an array of bytes that contains the data being queried or set. The size of the array is defined by the ANY_SIZE constant.
 */
 ...
 *
 * @param LinkSpeed This field stores the link speed in units of 100 bps. Zero indicates no change from the speed returned when the protocol called NdisRequest with OID_GEN_LINK_SPEED.
 * @param MaximumTotalSize This field stores the maximum number of bytes per packet that the protocol can send over the network. Zero indicates no change from the value returned when the protocol called NdisRequest with OID_GEN_MAXIMUM_TOTAL_SIZE.
 ...
 * @param ProtocolBuffer This field is a buffer containing protocol-specific information supplied by a higher-level component that makes connections through NDISWAN to the appropriate protocol(s). Maximum observed size is 600 bytes on Windows Vista, 1200 on Windows 10.
 */
 ...
 *
 * @param m_ValidFields This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: ETH_802_3_SRC_ADDRESS, ETH_802_3_DEST_ADDRESS, ETH_802_3_PROTOCOL.
 ...
 *
 * @param m_ValidFields This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: IP_V4_FILTER_SRC_ADDRESS, IP_V4_FILTER_DEST_ADDRESS, IP_V4_FILTER_PROTOCOL.
 ...
 *
 * @param m_ValidFields This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: IP_V6_FILTER_SRC_ADDRESS, IP_V6_FILTER_DEST_ADDRESS, IP_V6_FILTER_PROTOCOL.
 ...
 *
 * @param m_ValidFields This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: TCPUDP_SRC_PORT, TCPUDP_DEST_PORT, TCPUDP_TCP_FLAGS.
 ...
 *
 * @param m_ValidFields This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: ICMP_TYPE, ICMP_CODE.
 ...
 *
 * @param m_dwUnionSelector This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: ETH_802_3.
 ...
 *
 * @param m_dwUnionSelector This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: IPV4, IPV6.
 ...
 *
 * @param m_dwUnionSelector This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: TCPUDP, ICMP.
 ...
 * @param m_dwDirectionFlags This field stores direction flags. These flags can be used to indicate whether the filter applies to sent packets, received packets, or both.
 * @param m_FilterAction This field specifies the action to be taken when the filter conditions are met. It can be one of the following values: FILTER_PACKET_PASS, FILTER_PACKET_DROP, FILTER_PACKET_REDIRECT, FILTER_PACKET_PASS_RDR, FILTER_PACKET_DROP_RDR.
 * @param m_ValidFields This field stores the valid fields flags. These flags determine which fields in the structure are valid. The flags can be a combination of the following values: DATA_LINK_LAYER_VALID, NETWORK_LAYER_VALID, TRANSPORT_LAYER_VALID.
...
/**
 * @brief STATIC_FILTER_TABLE structure is used to define a table of static filters for the network adapter.
 *
 * @param m_TableSize This field stores the size of the table, i.e., the number of filters in the table.
 * @param m_StaticFilters This field is an array of STATIC_FILTER structures that define the filters in the table. The size of the array is defined by the ANY_SIZE constant, which allows for a variable-sized array.
 */
 ...
 *
 * @param fast_io_header This field is a volatile FAST_IO_SECTION_HEADER structure that stores the fast I/O write union and the read-in-progress flag.
 * @param fast_io_packets This field is an array of INTERMEDIATE_BUFFER structures that store the fast I/O packets. The size of the array is defined by the ANY_SIZE constant.
 */
 ...
 *
 * @param packets This field is a pointer to an array of PINTERMEDIATE_BUFFER structures. Each PINTERMEDIATE_BUFFER structure contains the packet data and related information.
